...
// Allow overriding the profile fetch timeout via env for slow networks
const PROFILE_TIMEOUT_MS = Number(import.meta.env.VITE_PROFILE_TIMEOUT_MS || 30000);
// Toggle profile fetch (DB hit) to avoid slowing down recipe loads; default off
const ENABLE_PROFILE_FETCH = (import.meta.env.VITE_ENABLE_PROFILE_FETCH || 'false') === 'true';
let profileTimeoutWarned = false;
...
const withTimeout = async (promise, ms, label = 'operation', abortController) => {
  let timeoutId;
  const timeoutPromise = new Promise((_, reject) => {
    timeoutId = setTimeout(() => {
      // Abort underlying request if possible so we don't leave hanging fetches
      if (abortController) {
...
async function handleUserProfile(user, onUserChange) {
  ...
  // Skip profile fetch entirely (use email/displayName) to keep UI snappy
